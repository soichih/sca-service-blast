...
dbtype=`cat $SCA_TASK_DIR_FASTA/products.json | underscore select '.fasta .type' --outfmt text`
dbtitle=sometitle
dbname=somedb
input_file=`cat $SCA_TASK_DIR_FASTA/products.json | underscore select '.fasta .filename' --outfmt text`
